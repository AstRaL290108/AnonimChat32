...
async def start_test(call: types.CallbackQuery, state: FSMContext):
	...
	await RegisterState.get_sex.set()
...
async def get_sex_func(call: types.CallbackQuery, state: FSMContext):
	...
	await RegisterState.get_age.set()
...
async def get_age_func(msg: types.Message, state: FSMContext):
	...
	try:
		...
		if age >= 8 and age <= 99:
			...
			await RegisterState.get_country.set()
...
async def get_country_func(call: types.CallbackQuery, state: FSMContext):
	...
	await RegisterState.get_room.set()
...
async def get_room_func(call: types.CallbackQuery, state: FSMContext):
	...
	await bot.send_message(chat_id, register_msg.ending, reply_markup = menu_btn.main_menu)

	db.insert_into({
		'table': 'users', 
		'user_id': chat_id,
		'username': f"{call.message.chat.username}",
		'name': f"{call.message.chat.full_name}",
		'room': call.data,
...
def register_handlers(dp: Dispatcher):
	dp.register_callback_query_handler(start_test, lambda c: c.data == "let_test", state = None)
	dp.register_callback_query_handler(get_sex_func, lambda c: c.data, state = RegisterState.get_sex)
	dp.register_message_handler(get_age_func, state = RegisterState.get_age)
	dp.register_callback_query_handler(get_country_func, lambda c: c.data, state = RegisterState.get_country)
	dp.register_callback_query_handler(get_room_func, lambda c: c.data, state = RegisterState.get_room)